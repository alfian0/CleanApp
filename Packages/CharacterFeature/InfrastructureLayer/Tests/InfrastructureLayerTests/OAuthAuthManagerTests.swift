...
final class OAuthAuthManagerTests: XCTestCase {
  ...
  func testValidToken_ThrowsError_WhenTokenIsExpired() throws {
    ...
    XCTAssertThrowsError(try authManager.validToken()) { error in
      XCTAssertEqual(
        (error as? OAuthSwiftError)?.localizedDescription,
        OAuthSwiftError.tokenExpired(error: nil).localizedDescription
      )
    }
  }
...
class MockOAuth2Swift: OAuth2Swift {
  ...
  override func renewAccessToken(
    withRefreshToken _: String,
    parameters _: OAuthSwift.Parameters? = nil,
    headers _: OAuthSwift.Headers? = nil,
    completionHandler completion: @escaping OAuth2Swift.TokenCompletionHandler
  ) -> (any OAuthSwiftRequestHandle)? {
...
class MockKeychainSwift: KeychainSwift {
  ...
  override func set(_ value: String, forKey key: String, withAccess _: KeychainSwiftAccessOptions? = nil) -> Bool {
    storage[key] = value
    return true
  }
