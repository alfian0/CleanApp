...
final class AuthHTTPClientDecoratorTests: XCTestCase {
    func test_validTokenWith401Response() async throws {
        ...
        do {
            ...
            XCTAssertEqual(spyHTTPClient.capturedRequests.first?.allHTTPHeaderFields?["Authorization"], "Bearer valid_token")
            XCTAssertEqual(spyHTTPClient.capturedRequests.last?.allHTTPHeaderFields?["Authorization"], "Bearer refresh_token")
        } catch {
            XCTFail()
        }
    }
